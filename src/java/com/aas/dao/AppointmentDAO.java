...
public class AppointmentDAO {
    ...
    public List<TimeSlot> getAvailableTimeSlots() {
        ...
        String sql = "SELECT ts.id, ts.advisor_id, u.name AS advisor_name, ts.date, ts.time_available, ts.location " +
                     "FROM time_slot ts JOIN user u ON ts.advisor_id = u.user_id " +
                     "WHERE ts.status = 'Available' ORDER BY ts.date, ts.time_available";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql);
             ResultSet rs = pstmt.executeQuery()) {
            while (rs.next()) {
                TimeSlot slot = new TimeSlot();
                slot.setScheduleId(rs.getInt("id"));
                ...
                slot.setStartTime(rs.getTime("time_available").toLocalTime());
    ...
    public boolean createAppointment(int studentId, int advisorId, int scheduleId, String appointmentType, String description) {
        String insertSql = "INSERT INTO appointment (student_id, advisor_id, schedule_id, appointment_type, description, status) VALUES (?, ?, ?, ?, ?, 'Pending')";
        String updateSlotSql = "UPDATE time_slot SET status = 'Booked' WHERE id = ?";
    ...
    public List<Appointment> getStudentAppointments(int studentId) {
        ...
        String sql = "SELECT a.*, ts.date, ts.time_available, ts.location " +
                     "FROM appointment a JOIN time_slot ts ON a.schedule_id = ts.id " +
    ...
    public List<Appointment> getPendingAppointmentsForAdvisor(int advisorId) {
        List<Appointment> list = new ArrayList<>();
        String sql = "SELECT a.*, u.name AS student_name, ts.date, ts.time_available, ts.location " +
                     "FROM appointment a JOIN user u ON a.student_id = u.user_id " +
                     "JOIN time_slot ts ON a.schedule_id = ts.id " +
    ...
    public List<Appointment> getAllAppointments() {
        List<Appointment> list = new ArrayList<>();
        String sql = "SELECT a.*, ts.date, ts.time_available, ts.location " +
                     "FROM appointment a JOIN time_slot ts ON a.schedule_id = ts.id ORDER BY a.created_at DESC";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql);
             ResultSet rs = pstmt.executeQuery()) {
            while (rs.next()) { list.add(mapRow(rs, false)); }
    ...
    public boolean rejectAppointmentWithReason(int appointmentId, String reason) {
        ...
        String releaseSql = "UPDATE time_slot SET status = 'Available' WHERE id = (SELECT schedule_id FROM appointment WHERE appointment_id = ?)";
        Connection conn = null;
        try {
            conn = DBConnection.getConnection();
            conn.setAutoCommit(false);
    ...
    public boolean editAppointmentDetails(int appointmentId, String newDesc) {
        String sql = "UPDATE appointment SET description = ? WHERE appointment_id = ? AND status = 'Pending'";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
    ...
    private Appointment mapRow(ResultSet rs, boolean includeStudentName) throws SQLException {
        ...
        apt.setAppointmentDate(rs.getDate("date").toLocalDate());
        apt.setStartTime(rs.getTime("time_available").toLocalTime());
